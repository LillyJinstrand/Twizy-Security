/******************************************************************************
 * Copyright 2017 The Apollo Authors. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *****************************************************************************/

#ifndef MODULES_CANBUS_VEHICLE_TWIZY_PROTOCOL_GEAR_AND_BREAKPEDAL_59B_H_
#define MODULES_CANBUS_VEHICLE_TWIZY_PROTOCOL_GEAR_AND_BREAKPEDAL_59B_H_

#include "modules/drivers/canbus/can_comm/protocol_data.h"
#include "modules/canbus/proto/chassis_detail.pb.h"

namespace apollo {
namespace canbus {
namespace twizy {

class Gearandbreakpedal59b : public ::apollo::drivers::canbus::ProtocolData<
                    ::apollo::canbus::ChassisDetail> {
 public:
  static const int32_t ID;
  Gearandbreakpedal59b();
  void Parse(const std::uint8_t* bytes, int32_t length,
                     ChassisDetail* chassis) const;

 private:

  // config detail: {'name': 'Brake_PedalStatus', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|0]', 'bit': 8, 'type': 'bool', 'order': 'intel', 'physical_unit': ''}
  bool brake_pedalstatus(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'Gear_R', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|0]', 'bit': 5, 'type': 'bool', 'order': 'intel', 'physical_unit': ''}
  bool gear_r(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'Gear_N', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|0]', 'bit': 3, 'type': 'bool', 'order': 'intel', 'physical_unit': ''}
  bool gear_n(const std::uint8_t* bytes, const int32_t length) const;

  // config detail: {'name': 'Gear_D', 'offset': 0.0, 'precision': 1.0, 'len': 1, 'is_signed_var': False, 'physical_range': '[0|0]', 'bit': 1, 'type': 'bool', 'order': 'intel', 'physical_unit': ''}
  bool gear_d(const std::uint8_t* bytes, const int32_t length) const;
};

}  // namespace twizy
}  // namespace canbus
}  // namespace apollo

#endif  // MODULES_CANBUS_VEHICL_TWIZY_PROTOCOL_GEAR_AND_BREAKPEDAL_59B_H_
